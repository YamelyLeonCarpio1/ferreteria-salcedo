...
const FORM_VACIO = { nombre: '', descripcion: '', precio: '', precioOferta: '', stock: '', stockMinimo: '5', sku: '', categoriaId: '', destacado: false }
...
export default function ProductosAdmin() {
  ...
  const abrirEditar = (p) => {
    setEditando(p.id)
    setForm({ nombre: p.nombre, descripcion: p.descripcion || '', precio: p.precio, precioOferta: p.precioOferta || '', stock: p.stock, stockMinimo: p.stockMinimo, sku: p.sku || '', categoriaId: p.categoriaId, destacado: p.destacado })
    setModal(true)
  }
  ...
  const handleGuardar = async (e) => {
    ...
    try {
      if (editando) {
        await axios.patch(`/api/admin/productos/${editando}`, form)
        toast.success('Producto actualizado')
      } else {
        await axios.post('/api/admin/productos', form)
        toast.success('Producto creado')
      }
      setModal(false); setEditando(null); setForm(FORM_VACIO); cargar()
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        ...
        <button onClick={() => { setModal(true); setEditando(null); setForm(FORM_VACIO) }} className="btn btn-rojo" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <tr key={p.id}>
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem' }}>
                    <img src={p.imagenes?.[0] || 'https://placehold.co/40x40'} alt="" style={{ width: '40px', height: '40px', objectFit: 'cover', borderRadius: '6px' }} />
                    <div>
          ...
          <div style={{ background: 'white', borderRadius: '12px', padding: '2rem', width: '100%', maxWidth: '560px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleGuardar}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
                ...
                <button type="button" onClick={() => setModal(false)} className="btn btn-gris" style={{ flex: 1, padding: '0.8rem' }}>Cancelar</button>
              </div>
            </form>
          </div>
        </div>
